...
/*Create table obs_concept_group for the obs_group_id*/
	...
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='72074743-ae89-425e-ba48-718b02585b91' 
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='bc514162-eae6-4126-95b4-c3695a410e2d'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs,openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='bc514162-eae6-4126-95b4-c3695a410e2d' 
	GROUP BY openmrs.obs.person_id,encounter_id;
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='40a0c708-a7bb-40ee-941b-c7bdefbb73bd'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs,openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='40a0c708-a7bb-40ee-941b-c7bdefbb73bd' 
	GROUP BY openmrs.obs.person_id,encounter_id;
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='369d0166-14dd-4f43-baa4-10ce377c5afb'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs,openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='369d0166-14dd-4f43-baa4-10ce377c5afb' 
	GROUP BY openmrs.obs.person_id,encounter_id;
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='cec53311-713b-4d80-8387-639be8d90f16'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs,openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='cec53311-713b-4d80-8387-639be8d90f16' 
	GROUP BY openmrs.obs.person_id,encounter_id;
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='654d4c9e-f171-485c-9aee-5c906406342c'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs,openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='654d4c9e-f171-485c-9aee-5c906406342c' 
	GROUP BY openmrs.obs.person_id,encounter_id;
   ...
   /*Answer*/
	INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,obs_group_id,value_coded,
		...
		INSERT INTO obs(person_id,concept_id,encounter_id,obs_datetime,location_id,creator,date_created,uuid)
	SELECT DISTINCT c.patient_id,(select concept_id from concept where uuid='0c362159-faae-4212-8866-5c59afc0c99b'),c.encounter_id,c.encounter_datetime,c.location_id,1,c.date_created,UUID()
	FROM encounter c, itech.encounter e, itech.adherenceCounseling ac 
	WHERE c.uuid = e.encGuid 
	AND e.patientID = ac.patientID 
	AND e.siteCode = ac.siteCode
	...
	SELECT MAX(openmrs.obs.obs_id) as obs_id,openmrs.obs.person_id,openmrs.obs.concept_id,openmrs.obs.encounter_id
	FROM openmrs.obs, openmrs.concept c
	WHERE openmrs.obs.concept_id=c.concept_id and c.uuid='0c362159-faae-4212-8866-5c59afc0c99b' 
	GROUP BY openmrs.obs.person_id,encounter_id;
